...
namespace GlobalSnowEffect {
    ...
    public class GlobalSnow : MonoBehaviour {
        ...
        [SerializeField]
        bool _allowBatchedMeshes;

        public bool allowBatchedMeshes {
            get {
                return _allowBatchedMeshes;
            }
            set {
                if (value != _allowBatchedMeshes) {
                    _allowBatchedMeshes = value;
        ...
        void RebuildCommandBuffer() {
            ...
            if (!scene.isLoaded) {
                ...
                // workaround: renderer.isVisible property needs some frame to update during initialization
                    ...
                    if (ic != null && ic.isActiveAndEnabled && ic.renderers != null) {
                        ...
                        for (int j = 0; j < rrCount; j++) {
                            ...
                            if (r.enabled && r.gameObject.activeInHierarchy) {
                                if (!excludedRenderersDict.ContainsKey(r)) {
                                    excludedRenderer.renderer = r;
                                    excludedRenderer.wasVisible = !ic.receiveSnow && (dirtyVisible || r.isVisible);
                                    GrowIfNeeded<ExcludedRenderer>(ref excludedRenderers, excludedRenderersCount);
                ...
                for (int k = 0; k < excludedRenderersCount; k++) {
                    ...
                    if (r != null && (dirtyVisible || r.isVisible) && r.enabled && r.gameObject.activeInHierarchy) {
                        AddCommandBufferRenderer(r, cbMatExcludedObjects);
                    }
                }
            }
            ...
            //if (boxRenderer == null) {
            //    boxRenderer = GameObject.Find("Cuby").GetComponent<Renderer>();
            //}
            //if (boxRenderer != null) {
            //    buf.SetViewProjectionMatrices(cameraEffect.worldToCameraMatrix, cameraEffect.projectionMatrix);
            //    buf.DrawMesh(boxRenderer.GetComponent<MeshFilter>().sharedMesh, boxRenderer.transform.localToWorldMatrix, cbMat);
            //}
        ...
        //Renderer boxRenderer; // TODO: Test direct global snow injection by renderers
        ...
        void AddCommandBufferRenderer(Renderer r, Material mat) {
            int subMeshCount = 0;
            if (r is SkinnedMeshRenderer) {
                ...
                if (mf != null && mf.sharedMesh != null) {
                    if (r.isPartOfStaticBatch && !_allowBatchedMeshes) {
                        // Renderer is part of a static batching, use the collider instead if allowBatchedMeshes is false
                        MeshCollider mc = r.GetComponent<MeshCollider>();
                        if (mc != null) {
                            Mesh colliderMesh = mc.sharedMesh;
                            if (colliderMesh != null) {
                                Matrix4x4 m = Matrix4x4.TRS(r.transform.position, r.transform.rotation, r.transform.lossyScale);
                                buf.DrawMesh(colliderMesh, m, mat);
                                return;
                            }
                        }
                    }
        ...
        void FindExcludedRenderersInScene(Scene scene) {
            if (Time.frameCount >= 5) { // repeat scan next frames so it gives time to initialization/load of other objects
            ...
            for (int k = 0; k < count; k++) {
                ...
                o.GetComponentsInChildren<Renderer>(true, tmpRenderers);
        ...
        void UpdateSnowCoverageNow() {
            ...
            zenithCam.backgroundColor = new Color(1f, 0f, 0f, 0f); // Color.white; // new Color(0.9882353f, 0.4470558f, 0.75f, 0f);
        ...
        void RenderTerrainMarks() {
            ...
            for (int blit = 0; blit < MAX_DECAL_BLITS_PER_FRAME; blit++) {
                ...
                for (int k = 0; k < VECTOR_ARRAY_LENGTH; k++) {
                    ...
                    if (zenithCam == null) {
                        worldPosArray[k] = new Vector4(vpos.x, 0, vpos.z, 0);
                    } else {
                        worldPosArray[k] = new Vector4(vpos.x, (zenithCam.transform.position.y - vpos.y) / zenithCam.farClipPlane, vpos.z, 0);
                    }
                    stamps++;
                }
                if (stamps == 0) break;
...
#if UNITY_EDITOR
                if (Camera.current.cameraType == CameraType.SceneView) { //.Equals("SceneCamera")) {
                    updateCoverage = true;
                }
#endif
            }
